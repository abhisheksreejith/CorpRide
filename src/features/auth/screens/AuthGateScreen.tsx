    ...
    const unsub = auth().onAuthStateChanged(async (user) => {
      ...
      try {
        ...
        try {
          ...
          const messaging = require('@react-native-firebase/messaging').default as any;
          if (messaging) {
            ...
            if (token) {
              await firestore().collection('users').doc(user.uid).collection('fcmTokens').doc(token).set({ createdAt: Date.now(), platform: Platform.OS }, { merge: true });
              try { console.log('FCM token saved:', token); } catch {}
            }
        ...
        } catch (e) { try { console.log('FCM setup error', e); } catch {} }
